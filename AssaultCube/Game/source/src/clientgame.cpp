...
static int lastTelemetryPrint = 0;
...
static FILE* eventsFile = NULL;
static int eventCounter = 0;
static char currentMatchId[32] = "M000";
static const char* currentPlayerId = "P001";
static char currentUsername[64] = "unknown";
static FILE* matchesFile = NULL;
static int matchCounter = 0;
static bool matchLogged = false;
static FILE* playersFile = NULL;
static bool playerLogged = false;
float matchStartTime = 0.0f;
void sendToFirebase(const char* path, const char* jsonData)
{
    // اكتب الـ JSON في ملف مؤقت
    FILE* tmpFile = NULL;
    fopen_s(&tmpFile, "data\\fb_tmp.json", "w");
    if (!tmpFile) return;
    fprintf(tmpFile, "%s", jsonData);
    fclose(tmpFile);

    // استخدم curl مع الملف بدل ما نكتب الـ JSON في الـ command
    char cmd[1024];
    sprintf_s(cmd,
        "curl -s -X PUT "
        "-H \"Content-Type: application/json\" "
        "-d @data\\fb_tmp.json "
        "\"https://sentrix-ai-cd8b6-default-rtdb.firebaseio.com%s\"",
        path);
...
{
    FILE* f = NULL;
    fopen_s(&f, "data\\current_user.txt", "r");
    if (f)
    {
        memset(currentUsername, 0, sizeof(currentUsername));
        fgets(currentUsername, sizeof(currentUsername), f);
        size_t len = strlen(currentUsername);
        if (len > 0 && (currentUsername[len - 1] == '\n' || currentUsername[len - 1] == '\r'))
            currentUsername[len - 1] = 0;
        if (len > 1 && currentUsername[len - 2] == '\r')
            currentUsername[len - 2] = 0;
        fclose(f);
    }
}
...
float getNearestEnemyDistance()
{
    ...
    loopv(players)
    {
        dynent* p = players[i];
        if (!p) continue;
        float dx = p->o.x - player1->o.x;
        float dy = p->o.y - player1->o.y;
        float dz = p->o.z - player1->o.z;
...
void updateworld(int curtime, int lastmillis)
{
    ...
    if (!sessionStarted && player1->state == CS_ALIVE)
    {
        matchStartTime = lastmillis / 1000.0f;
        sessionStarted = true;
        conoutf("SESSION STARTED");
    }

    if (sessionStarted && !matchLogged && matchesFile)
    {
        time_t now = time(NULL);
        tm localTime;
        localtime_s(&localTime, &now);

        sprintf_s(currentMatchId, "M%04d%02d%02d_%02d%02d%02d_%03d",
        ...
        char recordingDate[32];
        sprintf_s(recordingDate, "%04d-%02d-%02d %02d:%02d:%02d",
            localTime.tm_year + 1900,
            localTime.tm_mon + 1,
            localTime.tm_mday,
            localTime.tm_hour,
        ...
        char videoFilename[256] = "unknown.mp4";
        char videoPath[512] = "unknown_path";

        FILE* videoInfoFile = NULL;
        fopen_s(&videoInfoFile, "F:\\AC-master\\AC-master\\data\\video_info.txt", "r");

        if (videoInfoFile)
        {
            if (fgets(videoFilename, sizeof(videoFilename), videoInfoFile))
                videoFilename[strcspn(videoFilename, "\n")] = 0;

            if (fgets(videoPath, sizeof(videoPath), videoInfoFile))
                videoPath[strcspn(videoPath, "\n")] = 0;

            fclose(videoInfoFile);
        }

        // ---------------------- تعديل السلاح ليخرج الاسم ----------------------
        int weaponId = player1->gunselect;
        const char* weaponNames[] = { "Pistol", "MTP-57", "AK-47", "Sniper", "Shotgun" };
        const char* weaponNameStr = (weaponId >= 0 && weaponId < (int)(sizeof(weaponNames) / sizeof(weaponNames[0])))
            ? weaponNames[weaponId]
            : "Unknown";
        // -----------------------------------------------------------------------

        float currentTime = lastmillis / 1000.0f;
        float durationSec = currentTime - matchStartTime;
        fprintf(matchesFile,
            "%s,%s,legit,none,%s,%s,%s,%s,%.2f,30,1280x720,%s,Amir,auto-generated match\n",
            currentMatchId,
            currentPlayerId,
            videoFilename,
            videoPath,
            clientmap,
            weaponNameStr,   // ← اسم السلاح بدل الرقم
            durationSec,
            recordingDate);
        ...
        conoutf("MATCH LOGGED: %s", currentMatchId);
    }

    if (sessionStarted && !playerLogged && playersFile)
    {
        loadCurrentUser();
        fprintf(playersFile, "%s,\"%s\",legit,none,medium,%s,auto-generated player\n",
            currentPlayerId,
            currentUsername,
            currentUsername);
        fflush(playersFile);
        playerLogged = true;
        conoutf("PLAYER LOGGED: %s", currentPlayerId);
    }

    if (sessionStarted && !logFile)
    {
        fopen_s(&logFile, "data\\telemetry.csv", "a");
        ...
        if (logFile)
        {
            ...
            if (size == 0)
            {
                fprintf(logFile, "timestampSec,isShooting,playerPosX,playerPosY,playerPosZ,aimYawSpeed,aimPitchSpeed,moveSpeed,deltaYaw,deltaPitch,health,weaponId,isOnGround,distanceToNearestEnemy\n");
                fflush(logFile);
            }

            conoutf("FILE OPENED SUCCESS");
        }
        else
        {
            conoutf("FILE FAILED");
    ...
    if (lastmillis - lastTelemetryPrint >= 200)
    {
        float yawSpeed = angleDiff(player1->yaw, lastYaw);
        float pitchSpeed = fabs(player1->pitch - lastPitch);
        int health = max(0, player1->health);
        int weaponId = player1->gunselect;
        int isOnGround = player1->onfloor ? 1 : 0;
        float nearestEnemy = getNearestEnemyDistance();
        float dx = player1->o.x - lastX;
        float dy = player1->o.y - lastY;
        float dz = player1->o.z - lastZ;
        float moveSpeed = sqrt(dx * dx + dy * dy + dz * dz);

        float deltaYaw = angleDiff(player1->yaw, lastYaw);
        float deltaPitch = player1->pitch - lastPitch;

        float timestampSec = lastmillis / 1000.0f;
        int isShooting = player1->attacking ? 1 : 0;

        conoutf("T: %.2f | Shoot: %d | X: %.2f Y: %.2f Z: %.2f | YawSpeed: %.2f | PitchSpeed: %.2f | MoveSpeed: %.2f",
            timestampSec, isShooting,
            player1->o.x, player1->o.y, player1->o.z,
            yawSpeed, pitchSpeed, moveSpeed);

        if (logFile)
        {
            fprintf(logFile, "%.2f,%d,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%d,%d,%d,%.2f\n",
                timestampSec, isShooting,
                player1->o.x, player1->o.y, player1->o.z,
                yawSpeed, pitchSpeed, moveSpeed,
                deltaYaw, deltaPitch,
                health, weaponId, isOnGround, nearestEnemy);
        }
...
void dokill(playerent* pl, playerent* act, bool gib, int gun)
{
    ...
    if (eventsFile)
    {
        float eventTimestampSec = lastmillis / 1000.0f;

        // سجّل الـ kill لو player1 هو اللي قتل
        if (act == player1)
        {
            loadCurrentUser();
            const char* gunNameStr = valid_weapon(gun) ? gunnames[gun] : "unknown";

            // kill event
            fprintf(eventsFile, "E_%s_%03d,%s,%s,kill,%.2f,%s,%d,game_hook,player kill event\n",
                currentMatchId, eventCounter++,
                currentMatchId, currentPlayerId,
                eventTimestampSec, gunNameStr, pl->clientnum);

            // headshot = any gib
            if (gun == GUN_SNIPER)

            {
                fprintf(eventsFile, "E_%s_%03d,%s,%s,headshot,%.2f,%s,%d,game_hook,sniper headshot\n",
                    currentMatchId, eventCounter++,
                    currentMatchId, currentPlayerId,
                    eventTimestampSec, gunNameStr, pl->clientnum);
            }
        ...
        // سجّل الـ death لو player1 هو اللي مات
        if (pl == player1)
        {
            const char* gunNameStr = valid_weapon(gun) ? gunnames[gun] : "unknown";
            fprintf(eventsFile, "E_%s_%03d,%s,%s,death,%.2f,%s,%d,game_hook,player death event\n",
                currentMatchId, eventCounter++,
                currentMatchId, currentPlayerId,
                eventTimestampSec, gunNameStr, act->clientnum);
        }
    }
...
void startmap(const char* name, bool reset, bool norespawn)
{
    loadCurrentUser();
    matchLogged = false;
    sessionStarted = false;

    // اقفل الملفات القديمة وافتح جديدة باسم اللاعب الحالي
    if (eventsFile) { fclose(eventsFile); eventsFile = NULL; }
    if (matchesFile) { fclose(matchesFile); matchesFile = NULL; }

    char eventsPath[256];
    sprintf_s(eventsPath, "data\\events_%s.csv", currentUsername);
    fopen_s(&eventsFile, eventsPath, "a");
    if (eventsFile)
    {
        fseek(eventsFile, 0, SEEK_END);
        long size = ftell(eventsFile);
        if (size == 0)
        {
            fprintf(eventsFile, "event_id,match_id,player_id,event_type,event_timestamp_sec,weapon,target_id,source,notes\n");
            fflush(eventsFile);
        }
        conoutf("EVENTS FILE OPENED: %s", eventsPath);
    }

    char matchesPath[256];
    sprintf_s(matchesPath, "data\\matches_%s.csv", currentUsername);
    fopen_s(&matchesFile, matchesPath, "a");
    if (matchesFile)
    {
        fseek(matchesFile, 0, SEEK_END);
        long size = ftell(matchesFile);
        if (size == 0)
        {
            fprintf(matchesFile, "match_id,player_id,class,cheat_type,video_filename,video_path,map_name,primary_weapon,duration_sec,fps,resolution,recording_date,collector_name,notes\n");
            fflush(matchesFile);
        }
        conoutf("MATCHES FILE OPENED: %s", matchesPath);
    }

    if (!playersFile)
    {
        fopen_s(&playersFile, "data\\players.csv", "a");
        if (playersFile)
        {
            fseek(playersFile, 0, SEEK_END);
            long size = ftell(playersFile);
            if (size == 0)
            {
                fprintf(playersFile, "player_id,player_name,class,cheat_type,skill_level,collector_name,notes\n");
                fflush(playersFile);
            }
            conoutf("PLAYERS FILE OPENED");
        }
    }
